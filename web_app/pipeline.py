...
def extract_results_zip(archive_path: Path, destination: Path) -> Path:
    """Extract a MetaTox results zip into a viewer-ready output directory."""
    ...
    if extracted_files == 0:
        raise RuntimeError("The uploaded archive is empty.")

    compiled = sorted(destination.glob("*_CompileResults.tsv"))
    if not compiled:
        raise RuntimeError(
            "No *_CompileResults.tsv files were found in the archive. "
            "Upload a MetaTox results zip generated by this application."
        )
